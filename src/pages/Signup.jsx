import React, { useState, useRef, useCallback } from "react";
import { Link, useNavigate } from "react-router-dom";
import { UserPlus, Loader2, AlertCircle, CheckCircle2, XCircle, Eye, EyeOff } from "lucide-react";
import Reveal from "../components/Reveal.jsx";
import usePageMeta from "../hooks/usePageMeta.js";
import { useAuth } from "../context/AuthContext.jsx";
import { supabase } from "../lib/supabaseClient.js";

const USERNAME_RE = /^[a-zA-Z0-9_]{3,20}$/;

function extractMessage(err) {
  if (!err) return "";
  if (typeof err === "string") return err;

  // Supabase 504 timeout — SMTP took too long
  if (err.status === 504 || (typeof err.message === "string" && err.message.includes("504"))) {
    return "The email server is taking too long to respond. Please wait 30 seconds and try again.";
  }

  // Standard message field
  if (typeof err.message === "string" && err.message) return err.message;

  // Supabase sometimes uses error_description
  if (typeof err.error_description === "string") return err.error_description;

  // Nested error objects
  if (err.error && typeof err.error === "object") return extractMessage(err.error);

  // Fallback — never show {}
  return "Something went wrong — please wait a moment and try again.";
}

export default function Signup() {
  usePageMeta("/signup");
  const navigate = useNavigate();
  const { signUp } = useAuth();

  const [form, setForm] = useState({
    fullName: "", username: "", email: "", password: "",
  });
  const [showPw, setShowPw]           = useState(false);
  const [usernameState, setUState]    = useState("idle"); // idle|checking|available|taken|invalid
  const [formStatus, setFormStatus]   = useState("idle"); // idle|loading|error
  const [errorMsg, setErrorMsg]       = useState("");
  const debounceRef = useRef(null);

  const pwLen      = form.password.length;
  const pwStrength = pwLen === 0 ? "none" : pwLen < 8 ? "weak" : pwLen < 12 ? "fair" : "strong";

  // ── Field update ─────────────────────────────────────────────────────
  function update(field, value) {
    if (formStatus === "error") { setFormStatus("idle"); setErrorMsg(""); }
    setForm((f) => ({ ...f, [field]: value }));
    if (field === "username") checkUsername(value);
  }

  // ── Live username check ──────────────────────────────────────────────
  const checkUsername = useCallback((value) => {
    clearTimeout(debounceRef.current);
    if (!value)                    { setUState("idle");    return; }
    if (!USERNAME_RE.test(value))  { setUState("invalid"); return; }
    setUState("checking");
    debounceRef.current = setTimeout(async () => {
      const { data, error } = await supabase.rpc("is_username_available", { check_username: value });
      if (error) { setUState("idle"); return; }
      setUState(data ? "available" : "taken");
    }, 400);
  }, []);

  function fail(msg) { setFormStatus("error"); setErrorMsg(msg); }

  // ── Submit ────────────────────────────────────────────────────────────
  async function handleSubmit(e) {
    e.preventDefault();
    setFormStatus("idle");
    setErrorMsg("");

    if (!form.fullName.trim())                     return fail("Please enter your full name.");
    if (!form.username.trim())                     return fail("Please choose a username.");
    if (usernameState === "invalid")               return fail("Username: 3–20 characters, letters/numbers/underscores only.");
    if (usernameState === "taken")                 return fail("That username is taken — please choose another.");
    if (usernameState === "checking")              return fail("Still checking username availability — please wait a moment.");
    if (!form.email.trim())                        return fail("Please enter your email address.");
    if (!form.email.includes("@"))                 return fail("Please enter a valid email address.");
    if (!form.password)                            return fail("Please create a password.");
    if (form.password.length < 8)                  return fail("Password must be at least 8 characters long.");

    setFormStatus("loading");

    const { error } = await signUp({
      email:    form.email.trim(),
      password: form.password,
      username: form.username.trim(),
      fullName: form.fullName.trim(),
    });

    if (error) return fail(extractMessage(error));

    navigate("/verify", { state: { email: form.email.trim() } });
  }

  // ── Username status icon ─────────────────────────────────────────────
  const UsernameIcon = () => {
    if (usernameState === "checking")  return <Loader2   size={16} className="animate-spin text-concrete" />;
    if (usernameState === "available") return <CheckCircle2 size={16} className="text-emerald-600" />;
    if (usernameState === "taken" || usernameState === "invalid")
      return <XCircle size={16} className="text-rose-500" />;
    return null;
  };

  // ── Render ────────────────────────────────────────────────────────────
  return (
    <section className="min-h-screen flex items-center justify-center blueprint-grid py-32 px-6">
      <Reveal className="w-full max-w-md bg-paper rounded-2xl border border-ink/10 shadow-[0_20px_60px_-20px_rgba(27,42,61,0.25)] p-8 sm:p-10">

        <div className="flex items-center gap-2.5 mb-1">
          <UserPlus className="text-blueprint" size={22} strokeWidth={1.6} />
          <p className="sheet-label text-brass">SHEET R-001 — REGISTRATION</p>
        </div>
        <h1 className="font-display text-3xl font-semibold text-blueprint mt-2">Create an account</h1>
        <p className="text-concrete text-sm mt-1">Track your enquiries and pick up where you left off.</p>

        <form onSubmit={handleSubmit} className="space-y-5 mt-8" noValidate>

          {/* Full name */}
          <div>
            <label className="sheet-label text-concrete block mb-2">Full name *</label>
            <input
              type="text"
              value={form.fullName}
              onChange={(e) => update("fullName", e.target.value)}
              placeholder="e.g. Salman Raza"
              className="w-full rounded-md border border-ink/15 px-4 py-3 text-sm focus:outline-none focus:border-blueprint transition-colors bg-paper"
            />
          </div>

          {/* Username */}
          <div>
            <label className="sheet-label text-concrete block mb-2">Username *</label>
            <div className="relative">
              <input
                type="text"
                value={form.username}
                onChange={(e) => update("username", e.target.value)}
                placeholder="e.g. salman_raza"
                autoCapitalize="none"
                autoCorrect="off"
                spellCheck={false}
                className="w-full rounded-md border border-ink/15 px-4 py-3 pr-10 text-sm focus:outline-none focus:border-blueprint transition-colors bg-paper"
              />
              <span className="absolute right-3 top-1/2 -translate-y-1/2">
                <UsernameIcon />
              </span>
            </div>
            {usernameState === "available" && <p className="text-emerald-600 text-xs mt-1">Username is available ✓</p>}
            {usernameState === "taken"     && <p className="text-rose-500 text-xs mt-1">That username is already taken.</p>}
            {usernameState === "invalid"   && <p className="text-rose-500 text-xs mt-1">3–20 characters: letters, numbers, underscores only.</p>}
          </div>

          {/* Email */}
          <div>
            <label className="sheet-label text-concrete block mb-2">Email address *</label>
            <input
              type="email"
              value={form.email}
              onChange={(e) => update("email", e.target.value)}
              placeholder="you@email.com"
              className="w-full rounded-md border border-ink/15 px-4 py-3 text-sm focus:outline-none focus:border-blueprint transition-colors bg-paper"
            />
          </div>

          {/* Password */}
          <div>
            <label className="sheet-label text-concrete block mb-2">Password *</label>
            <div className="relative">
              <input
                type={showPw ? "text" : "password"}
                value={form.password}
                onChange={(e) => update("password", e.target.value)}
                placeholder="At least 8 characters"
                className="w-full rounded-md border border-ink/15 px-4 py-3 pr-11 text-sm focus:outline-none focus:border-blueprint transition-colors bg-paper"
              />
              <button
                type="button"
                onClick={() => setShowPw((v) => !v)}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-concrete hover:text-ink transition-colors"
                aria-label={showPw ? "Hide password" : "Show password"}
              >
                {showPw ? <EyeOff size={16} /> : <Eye size={16} />}
              </button>
            </div>

            {/* Strength bar */}
            {pwStrength !== "none" && (
              <div className="mt-2 space-y-1">
                <div className="flex gap-1">
                  {[0,1,2].map((i) => (
                    <div key={i} className={`h-1.5 flex-1 rounded-full transition-colors duration-300 ${
                      pwStrength === "weak"   && i === 0 ? "bg-rose-500"    :
                      pwStrength === "fair"   && i <= 1  ? "bg-amber-400"   :
                      pwStrength === "strong"            ? "bg-emerald-500" : "bg-ink/10"
                    }`} />
                  ))}
                </div>
                <p className={`text-xs font-medium ${
                  pwStrength === "weak"   ? "text-rose-500"   :
                  pwStrength === "fair"   ? "text-amber-500"  : "text-emerald-600"
                }`}>
                  {pwStrength === "weak"   ? "Too short — minimum 8 characters" :
                   pwStrength === "fair"   ? "Good — longer is stronger"        :
                   "Strong password ✓"}
                </p>
              </div>
            )}
          </div>

          {/* Error message */}
          {formStatus === "error" && errorMsg && (
            <div className="flex items-start gap-2.5 bg-rose-50 border border-rose-200 rounded-lg px-3 py-3 text-rose-700 text-sm">
              <AlertCircle size={16} className="mt-0.5 shrink-0" />
              <span>{errorMsg}</span>
            </div>
          )}

          {/* Submit */}
          <button
            type="submit"
            disabled={formStatus === "loading"}
            className="w-full inline-flex flex-col items-center justify-center gap-1 rounded-full bg-blueprint text-paper px-7 py-3.5 text-sm font-semibold hover:bg-blueprint-deep transition-colors disabled:opacity-60 mt-1"
          >
            {formStatus === "loading" ? (
              <>
                <span className="inline-flex items-center gap-2">
                  <Loader2 size={16} className="animate-spin" /> Creating account…
                </span>
                <span className="text-paper/60 text-xs font-normal">Sending confirmation email, please wait…</span>
              </>
            ) : "Create account →"}
          </button>
        </form>

        <p className="text-concrete text-sm text-center mt-7">
          Already have an account?{" "}
          <Link to="/login" className="text-blueprint font-medium border-b border-blueprint/30 hover:border-blueprint transition-colors">
            Log in
          </Link>
        </p>
      </Reveal>
    </section>
  );
}
